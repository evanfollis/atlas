...
- **2a — register + store. DONE + DEPLOYED.** `models/prediction.py` (bucketed id, frozen spec, conservative null default, horizon validation), `storage/prediction_store.py` (append-only, dedup-on-read), `runner.register_predictions()` hooked into `run_cycle` after the scan, `prediction.registered` telemetry. Verified live: **20 forward predictions/cycle** (2 skipped), idempotent per horizon bucket, windows fully forward. `predictions.jsonl` gitignored.
  ...
  - ~~**Known limitation: lag not in tags**~~ **FIXED (2026-07-11)**: `from_autocorrelation_signal` now emits a `lag_N` tag, so `_build_signal_from_hypothesis` replays the lag the claim states instead of defaulting to lag=1. Claim and executed strategy now agree. Existing open predictions (registered before the fix) keep their frozen lag-1 spec; new buckets carry the correct lag.

- **2b — score → `live_observation` evidence. DONE + DEPLOYED (commit cf76b7b).** `runner.score_due_predictions()` runs every cycle: for each `predictions.list_due(now)` it fetches OHLCV via `since=` (fresh window-covering fetch, bypassing the stale main scan CSV), replays the **frozen** `strategy_tags` on `[window_start − warmup, resolve_ts]`, scores `run_backtest` on the **window only** at `fee_bps=26`, writes a **MODERATE-capped** `live_observation` Evidence (`experiment_id=prediction.id`), and resolves the prediction append-only via `model_copy`. **Verified live**: bucket 2948 (20 predictions) scored autonomously on deploy → 18 `confirmed_null` / 1 `inconclusive` / 1 `edge_appeared`, 19 moderate + 1 weak, **0 strong** (cap held → no spurious promotion). The honest expected result: on 1h price features after fees the refutations hold forward. `prediction.resolved` telemetry emitted. Original guardrails, all implemented:
  ...
  - **S1-P3 reconciliation (second write path to `.atlas/evidence/`).** `run_experiment` and `score_due_predictions` both write `Evidence`, with **divergent `experiment_id` schemes**: real Experiment record ids vs `prediction.id` (ledger evidence has **no backing Experiment record**). Per CLAUDE.md S1-P3 this is ADR-class and every joining consumer was checked: `_fresh_tested_datasets` (`runner.py:522`) does `if not exp_data: continue`; `cli.py:174/257` take a user-supplied id and `return` on None; `ingest.py:122` creates-if-missing and processes its own ids, never ledger evidence. **Verdict: benign** — every evidence→experiment join already tolerates experiment-less evidence, so no consumer breaks. The promotion gate never joins (it only counts unique `experiment_id`s among STRONG support, and ledger evidence is MODERATE). If a *new* consumer joins evidence→experiment, it must handle the missing-Experiment case.
  - **Degenerate Brier (note for 2c)**: `predicted_prob_up` is always 0.5 (a null forecast), so `brier_score` is always 0.25 — a dead metric as stored. **2c must key calibration on the `outcome` distribution (confirmed_null rate), not Brier** — or carry a real directional probability at registration when 2c is built.

- **2c — `atlas calibration` CLI. AFTER 2b.** Hit-rate / **outcome distribution (not Brier — see above)** across resolved predictions, per claim-type. Emit a distinct register/resolve telemetry split so "honest idle" is distinguishable from "honest productive-but-no-promotion."
