...
type: front-door
updated: 2026-07-02T02-20-00Z
---

# CURRENT_STATE — atlas

**Last updated**: 2026-07-02T02:20Z — reflection pass (idle window). **NO ATTENDED SESSIONS** (~74h since last attended, 2026-06-28T04:08Z). Loop state unchanged: `hypothesis_space_exhausted`, `evidence=253` frozen, `signals_found=22`, `skipped_not_promotable=5`. **PREDICTION BUCKET UPDATE**: runner crossed bucket boundary this window. `open_predictions=40` (bucket 2948: 20 predictions, window 2026-07-02→2026-07-09; bucket 2949: 20 NEW predictions, window 2026-07-09→2026-07-16). **CRITICAL: Phase 2b must ship before 2026-07-09 (~7 days)** — scorer must handle multiple independent buckets (score 2948 after Jul 9, score 2949 after Jul 16). Autocorrelation lag/tag mismatch (signals.py) — **9th carry-forward**, must fix in 2b session BEFORE scorer is written (tag format must match scorer expectation). **STALE URGENTS pending delete (9th carry-forward)**: `runtime/.handoff/URGENT-atlas-frozen-loop-2026-06-11T20-26Z.md` and `URGENT-atlas-unpushed-commits-3rd-cycle.md`. **BRANCH 7 AHEAD of origin** (7 reflect auto-commits, all CURRENT_STATE.md; push needed). S3-P2 gate silenced until 2b produces decisive outcome. — **PRIOR ATTENDED SESSION (2026-06-28T04:08Z)** summary retained below. **ROUTING FIX SHIPPED (commit f24d298)**: `_maybe_escalate_frozen_loop` now writes to `supervisor/handoffs/INBOX/` instead of `runtime/.handoff/`. The URGENT files the runner generated were invisible to the executive session because both paths were disjoint. `HANDOFF_DIR` renamed to `SUPERVISOR_HANDOFF_DIR` in runner.py; tests updated (4 references). Tests 180→**180** (all pass). Pushed. Note: existing stale URGENT at `runtime/.handoff/URGENT-atlas-frozen-loop-2026-06-11T20-26Z.md` is NOT moved — the gate is latched (`emitted_for_current_streak: true`) so the runner won't re-emit anyway; executive should delete or acknowledge it manually. — **Last attended session (2026-06-28T04:08Z)**: strip theater + forward-prediction ledger. **PRINCIPAL DIRECTION (2026-06-28): strip theater → forward-prediction ledger.** Both shipped this session; origin in sync (HEAD 1bbaf17). **PHASE 2a SHIPPED + DEPLOYED (commits a3cf8a8 + 1bbaf17, runner restarted 04:06:05Z)**: forward-prediction ledger foundation. Each cycle now registers dated, falsifiable FORWARD forecasts for detected signals — `models/prediction.py` (bucketed id = one prediction per claim per non-overlapping horizon window, idempotent across the hourly cycle; frozen strategy spec; conservative null default; horizon validation), `storage/prediction_store.py` (append-only, dedup-on-read), `runner.register_predictions()` + `prediction.registered` telemetry. `predictions.jsonl` gitignored. **Replayability allowlist (`REPLAYABLE_METHODS`)**: only register signals whose strategy is fully tag-encoded (so 2b can replay faithfully); cross-asset/lead-lag (proxy) and composite/calendar (generic-momentum fallback) are skipped — they don't represent their claim. Verified live: **20 forward predictions/cycle, 2 skipped** (window 2026-07-02..07-09), idempotent on re-run. Tests 173→**180**. Two codex reviews filed (`.reviews/strip-theater-…`, `.reviews/ledger-2a-…`); float-stringification footgun + reconstruction-faithfulness both fixed. **NEXT (specced in `CAUSAL_LOOP_AUDIT.md` §Implementation log): 2b** = score due predictions → `live_observation` evidence (replay frozen spec on the forward window only; conservative Brier quality mapping — expect `confirmed_null`, NOT promotions, on current 1h features; append-only resolution). **2c** = `atlas calibration` CLI. First scoreable window resolves **2026-07-09**. **Funding/OI feature expansion** (principal-chosen alt-data, no creds) is the Shape-4 follow-on where a real forward edge becomes plausible. — **PHASE 1 (commit ec8f4d0, restarted 03:41:19Z)**: confounder-search theater removed. `from_graph_gaps()` no longer spawns un-executable confounder follow-ups from refuted nodes; `backfill_falsified_claims()` no longer projects `confounder_search`-tagged hypotheses into the map. Deployed-state cleanup: pruned 4 dishonest confounder nodes (graph 73→**69** refuted, 4→**0** edges, restoring the honest backfill state — this also resolves the 138-line graph drift), migrated 5 FORMULATED confounder hypotheses → INFEASIBLE. Tests 172→**173**. Codex adversarial review filed (`.reviews/strip-theater-hypotheses-2026-06-28.md`). Verified live: cycle now emits `no_action` (honest idle), map holds at 69. — **EFFECT ON OPEN ITEMS**: the loop is now **honestly idle** (no theater, no map pollution); because no decisive cycles occur, the S3-P2 gate stays latched silent → **the frozen-loop URGENT churn stops** (`URGENT-atlas-frozen-loop-2026-06-11T20-26Z.md` theater root-cause fixed; residual idle is the expected interim state pending the ledger — leave until Phase 2 ships). P2 `no_claim_faithful_dataset` carry-forward is **subsumed** (that skip only fired on the now-removed theater hypotheses). — **PHASE 2 IN PROGRESS**: forward-prediction ledger (dated falsifiable forecasts → scored vs realized data → `live_observation` evidence + calibration). The real fresh-evidence engine; un-exhaustible because forward time always supplies new data. See `CAUSAL_LOOP_AUDIT.md` Q4/Q5.
...
- **Test count** — 168 after 71224e9 (P1 +12). No new tests in this window. No code changed.
