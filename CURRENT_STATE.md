...
type: front-door
updated: 2026-07-05T02-19-48Z
---

# CURRENT_STATE — atlas

**Last updated**: 2026-07-05T02:19Z — reflection pass (idle window). **NO ATTENDED SESSIONS** (~166h since last attended, 2026-06-28T04:08Z). Loop state unchanged: `hypothesis_space_exhausted`, `evidence=253` frozen, `signals_found=22`, `skipped_not_promotable=5`, `consecutive_empty_count=~515`. **PREDICTION BUCKETS**: 20 predictions in bucket 2948 (resolve 2026-07-09T00:00Z), 20 in bucket 2949 (resolve 2026-07-16), `open_total=40`. **CRITICAL: Phase 2b must ship before 2026-07-09 (~3d 21h = ~93.7h)**. **LAG TAG GAP**: all 40 existing predictions lack `lag_N` tags — the 2b scorer MUST parse lag from claim text (`re.search(r'lag (\d+)', claim)`) for existing bucket 2948/2949 predictions; tags-only approach fails. Autocorrelation lag/tag mismatch in signals.py — **15th carry-forward**, fix both: add tag in signals.py AND add claim-text fallback in scorer. `prediction.registered` reports `skipped_unreplayable: 2` every cycle — 2/22 signals discarded as non-replayable; `REPLAYABLE_METHODS` filter is silent (no log of which methods) — **5th carry-forward**. **STALE URGENTS pending delete (15th carry-forward)**: `runtime/.handoff/URGENT-atlas-frozen-loop-2026-06-11T20-26Z.md` and `URGENT-atlas-unpushed-commits-3rd-cycle.md`. **UNREAD TICK HANDOFF**: `runtime/.handoff/general-atlas-tick-complete-2026-06-28T04-51-01Z.md` not marked .done — escalation routing fix (f24d298) may not be deployed; runner restart required before Phase 2b. **BRANCH 13 AHEAD of origin** (13 reflect auto-commits, all CURRENT_STATE.md; push needed). S3-P2 gate silenced until 2b produces decisive outcome.
