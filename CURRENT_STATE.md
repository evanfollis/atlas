...
**Last updated**: 2026-04-29T17:00Z — Pool-rotation diagnosis surfaced as principal-class decision (`general-atlas-pool-rotation-decision-needed-2026-04-29T17-00Z.md`). Verified: 60 falsified, 14 formulated (never consumed), 2 stuck testing (BitMEX/KrakenFutures funding-rate, no data path). Loop is now a 2-hypothesis perpetual-continue machine. Net knowledge gain across recent cycles: zero. Tests 128.
...
- **Hypothesis pool rotation gap** (open, principal-class): runner only evaluates `testing` hypotheses each cycle. Currently the only 2 testing hypotheses are BitMEX/KrakenFutures funding-rate variants (`10dc7fca3973e82a`, `4fdf3a65763ab083`) that cannot execute because atlas has no reach to those exchanges from the Hetzner US host. 14 formulated hypotheses sit unconsumed. Net new evidence per cycle: zero. Three options (A: auto-promote with feasibility check + INFEASIBLE status; B: manual queue management; C: deprecate stuck hypotheses) are framed in `general-atlas-pool-rotation-decision-needed-2026-04-29T17-00Z.md`. Recommendation: A+C. No code change without principal direction.
...
- **Hypothesis pool rotation via signal scanner only** (open, 3rd carry-forward): runner bypasses 14 existing `formulated` hypotheses and always generates fresh hypotheses from the signal scanner. Pool now **76 total (60 falsified, 14 formulated, 2 stuck testing)**. Cycle now evaluates only **2 hypotheses** (was 5): only 2 remain in `testing`; the 14 formulated are never consumed. Kill cycles cause hypothesis IDs to change; `signals_found` stays at 22 regardless of data refresh. This is now blocking research progress, not just a backlog concern (4th carry-forward — principal-class, escalating).
...
2. **Hypothesis scheduling + stuck-testing orphans**: runner now selects only the same 2 hypotheses (2 remaining in `testing`); 14 formulated hypotheses never evaluated. Two hypotheses (`10dc7fca`, `4fdf3a65`) are permanently stuck in `testing` (funding-rate data unavailable). Both issues diagnosed in reflection 2026-04-26T14:21Z. Principal question: should the runner rotate through the formulated pool, and should stuck `testing` hypotheses time out?
3. Run `.venv/bin/python -m pytest` to confirm **128/128** baseline (127 → 128 after 61542d3).
4. **`/review` the S3-P2 escalation gate — URGENT (5th carry-forward, redesign territory)**: `_maybe_escalate_frozen_loop` has had 5 distinct bug classes in 5 commits over 3 days. Three interacting timestamps + semantic lambda in a generator — no single pass has been bug-free for >24h. Freeze and run `adversarial-review.sh` before next touch. `/review` EROFS workaround valid (see `supervisor/scripts/lib/adversarial-review.sh`).
