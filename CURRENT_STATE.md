...
type: front-door
updated: 2026-07-07T14-19-38Z
---

# CURRENT_STATE — atlas

**Last updated**: 2026-07-07T14:19Z — reflection pass (idle window). **NO ATTENDED SESSIONS** (~234h since last attended, 2026-06-28T04:08Z). Loop state unchanged: `hypothesis_space_exhausted`, `evidence=253` frozen, `signals_found=22`, `skipped_not_promotable=5`, `consecutive_empty_count=574` (from `.atlas/escalation_state.json`; +12 this window). **PREDICTION BUCKETS**: bucket 2949 registration active (window 2026-07-16); bucket 2948 (window 2026-07-09) CLOSED — **33.7h until resolve**. `open_total=40`. **CRITICAL: Phase 2b must ship before 2026-07-09T00:00Z (~33.7h = ~2.8 windows). LAST REFLECTION BEFORE MISS.** **LAG TAG GAP**: all 40 existing predictions lack `lag_N` tags — **20th carry-forward**. Scorer must add claim-text fallback `re.search(r'lag (\d+)', claim)` for existing records. **NEW: `skipped_unreplayable: 2` every cycle** — 2 predictions silently skip registration each cycle; cause unknown; investigate before scorer ships. **STALE URGENTS pending delete (20th carry-forward)**: `runtime/.handoff/URGENT-atlas-frozen-loop-2026-06-11T20-26Z.md` and `URGENT-atlas-unpushed-commits-3rd-cycle.md`. **UNREAD TICK HANDOFF**: `runtime/.handoff/general-atlas-tick-complete-2026-06-28T04-51-01Z.md` now **~229h unread** (12th consecutive window); escalation routing fix (f24d298) still not deployed; runner restart required before Phase 2b ships. **BRANCH 18 AHEAD of origin** (18 reflect auto-commits; push needed). S3-P2 gate silenced (`emitted_for_current_streak: true`). **20th consecutive idle window.**
