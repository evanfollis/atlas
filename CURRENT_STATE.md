...
**Last updated**: 2026-04-30T17:00Z — combined pool-rotation + signal-hash-drift decision now in `general-atlas-pool-rotation-v2-with-signal-drift-2026-04-30T17-00Z.md` (supersedes 04-29 handoff). Two independent blockers now framed (BitMEX data + signal-hash-drift); 4 options × 2 dataset-strictness sub-options. Recommendation: A+C+D2. Telemetry-blindness fix (9708867) holding.
...
- **RUNNER STUCK — two independent blockers (open, principal-class)**: (1) BitMEX/KrakenFutures data unavailable for 2 stuck `testing` hypotheses; (2) signal-hash-drift surfaced by synthesis 04-30T15:26Z — the 12 `formulated` hypotheses have parameter values measured Apr 18-19 that today's signal scans no longer match (different parameter values → different claim hashes → empty candidate list → early exit). Combined v2 decision handoff: `general-atlas-pool-rotation-v2-with-signal-drift-2026-04-30T17-00Z.md` (supersedes 04-29 handoff). Options A/B/C/D × strictness D1/D2; recommendation A+C+D2. NOT touching runner code without principal direction.
