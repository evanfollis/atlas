...
- **RUNNER STUCK — third mode (2026-05-02T14:18Z reflection): TESTING orphans + FORMULATED pool blocked — 14+ cycles, URGENT filed**: `hypotheses_evaluated: 0` on all 14+ cycles since A+C+D2 deploy. Old S3-P2 gate suppressed (last_emitted_ts Apr 27 predates current stuck period); new counter gate undeployed. URGENT handoff at `runtime/.handoff/URGENT-atlas-frozen-loop-2026-05-02T14-18-55Z.md`. Two independent causes: (1) **7 TESTING hypotheses orphaned** — `run_cycle` only evaluates hypotheses from current signal scan + FORMULATED top-up; TESTING hypotheses whose originating signals no longer fire are never selected (`runner.py:1081–1140`); (2) **5 FORMULATED hypotheses structurally blocked** — 2 ETH/BTC 4h hypotheses reference a timeframe intentionally excluded from DEFAULT_UNIVERSE (permanent, not reversible) but `_claim_is_permanently_infeasible` only checks exchange tokens, not excluded timeframes; 3 SOL 1h hypotheses have insufficient bars (genuinely reversible, months away). Fix: (a) add TESTING-hypothesis inclusion path in `run_cycle`; (b) extend `_claim_is_permanently_infeasible` to catch excluded timeframes. See reflection `atlas-reflection-2026-05-02T02-18-51Z.md` proposals P1+P3.
...
0. **URGENT HANDOFF** — `runtime/.handoff/URGENT-atlas-frozen-loop-2026-05-02T14-18-55Z.md`: 14+ empty cycles, old gate suppressed, new gate undeployed, principal action needed.
...
2. **RUNNER STILL STUCK — third frozen-loop mode (reflection 2026-05-02T02:18Z)**: 7 TESTING hypotheses are orphaned (`run_cycle` never evaluates TESTING hypotheses unless current signals match their claims — they don't). Fix: add TESTING hypothesis inclusion path in `run_cycle` at `runner.py:1091–1110`. This is P1 in the reflection and does NOT require adversarial review (it doesn't touch `_maybe_escalate_frozen_loop`). Also: 2 of the 5 FORMULATED hypotheses reference 4h timeframe (intentionally excluded from DEFAULT_UNIVERSE) — extend `_claim_is_permanently_infeasible` to mark these INFEASIBLE (`runner.py:205–220`, P3 in reflection).
3. **Test baseline**: 156 tests. Verify with `.venv/bin/python -m pytest` before any commit.
4. **Migration merge-collapse** (5th carry-forward): `scripts/migrate_claim_hash.py` merge path discards non-claim fields. `--allow-merge` gate aborts on detection but merge-allowed behavior is untested. Add fixture with differing `rationale` fields.
