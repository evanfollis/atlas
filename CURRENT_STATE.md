...
type: front-door
updated: 2026-07-06T02-18-21Z
---

# CURRENT_STATE — atlas

**Last updated**: 2026-07-06T02:18Z — reflection pass (idle window). **NO ATTENDED SESSIONS** (~192h since last attended, 2026-06-28T04:08Z). Loop state unchanged: `hypothesis_space_exhausted`, `evidence=253` frozen, `signals_found=22`, `skipped_not_promotable=5`, `consecutive_empty_count=539` (from `.atlas/escalation_state.json`). **PREDICTION BUCKETS**: registration now advances to bucket 2949 (window 2026-07-16) — bucket 2948 (window 2026-07-09) is CLOSED TO NEW REGISTRATIONS; 20 predictions already registered, scoring only. `open_total=40`. **CRITICAL: Phase 2b must ship before 2026-07-09T00:00Z (~69.7h = ~5.8 reflection windows)**. **LAG TAG GAP**: all 40 existing predictions lack `lag_N` tags — **17th carry-forward**. `skipped_unreplayable: 2` every cycle, `REPLAYABLE_METHODS` filter silent — **6th carry-forward**. **STALE URGENTS pending delete (17th carry-forward)**: `runtime/.handoff/URGENT-atlas-frozen-loop-2026-06-11T20-26Z.md` and `URGENT-atlas-unpushed-commits-3rd-cycle.md`. **UNREAD TICK HANDOFF**: `runtime/.handoff/general-atlas-tick-complete-2026-06-28T04-51-01Z.md` now **~190h unread** (9th consecutive window); escalation routing fix (f24d298) still not deployed; runner restart required. **BRANCH 15 AHEAD of origin** (15 reflect auto-commits; push needed). S3-P2 gate silenced (`emitted_for_current_streak: true`). **17th consecutive idle window.** Note: telemetry captured 3 cycles (not 12) in this window — partial tail read, not a runner degradation; event pattern healthy. Predictions now in bucket 2949; 2948 window is the scoring target.
